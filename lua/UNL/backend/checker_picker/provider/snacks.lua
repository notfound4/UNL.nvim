-- lua/UNL/backend/checker_picker/provider/snacks.lua
local M = { name = "snacks" }
...
function M.run(spec)
  ...
  if spec.on_submit then
    snacks_opts.actions.unl_submit = function(picker)
      local ret = {}
      for _, item in ipairs(picker.list.selected) do
        table.insert(ret, item.unl_value)
      end
  ...
  local default_check = spec.default_check and true or false
